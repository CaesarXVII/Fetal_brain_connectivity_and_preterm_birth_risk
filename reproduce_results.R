...
out <- Causal_mat(data_scaled = data_scaled,seed = 1345,method = "ges")

obs <- MEC_to_obs(nets = out$nets) # MEC stays for Markov equivalence class

# Global test on connectivity

stud_res_MEC <- stud_perm_test(obs = obs$T_ac,id_group = ptb_risk,alternative = "less",alpha = 0.05,seed = seed,n_sim = n_sim,surplus = surplus)
